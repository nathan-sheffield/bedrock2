...
Lemma swap_chars_over_uart_correct m mc :
  WeakestPrecondition.cmd (fun _ _ _ _ _ _ => False) swap_chars_over_uart nil m map.empty mc
  (fun t m l mc => True).
Proof.
  repeat t.
  eexists _, _, (fun v t _ l _ => exists p, map.of_list_zip ["running"; "prev"; "one"; "dot"]%string [v; p; word.of_Z(1); word.of_Z(46)] = Some l ); repeat t.
  eexists _, _, (fun v t _ l _ => exists rxv, map.putmany_of_list_zip ["polling"; "rx"]%string [v; rxv] l0 = Some l); repeat t.
  eexists _, _, (fun v t _ l _ => exists txv, map.putmany_of_list_zip ["polling"; "tx"]%string [v; txv] l0 = Some l); repeat t.
...
Lemma echo_server_correct m mc:
  WeakestPrecondition.cmd (fun _ _ _ _ _ _ => False) echo_server nil m map.empty mc
  (fun t m l mc => echo_server_spec t None).
Proof.
  repeat t.
  eexists _, _, (fun v t _ l _ => map.of_list_zip ["running"; "one"]%string [v; word.of_Z(1)] = Some l /\ echo_server_spec t None ); repeat t.
  { repeat split. admit. (* hfrosccfg*) }
  eexists _, _, (fun v t _ l _ => exists rxv, map.putmany_of_list_zip ["polling"; "rx"]%string [v; rxv] l0 = Some l /\
